...
class AnalysisQueryParameters(BaseQueryParameters):
    def __init__(
            self,
            # Fastq id
            name: Optional[str] = Query(
                None,
                alias="name",
                description="The Name to filter by, use <code>name[]</code> for multiple values"
            ),
            name_list: Optional[List[str]] = Query(
                None,
                alias="name[]",
                description=None,
        ...
        # Initialise analysis query parameters
        self.name = name
    ...
    def validate_query(self):
        ...
        if self.name is not None and self.name_list is not None:
            raise HTTPException(
                status_code=400,
                detail="Only one of analysisName or analysisName[] is allowed"
            )
        ...
        # # and created_before is specified, that created_after is less than created_before
